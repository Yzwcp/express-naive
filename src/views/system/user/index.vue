...
<template>
	<CommonPage>
		...
		<MeModal ref="modalRef" width="520px">
			...
			<n-alert v-if="modalAction === 'add'" type="warning" closable> 详细信息需由用户本人补充修改 </n-alert>
		</MeModal>
	</CommonPage>
</template>
...
const columns = [
		...
		render: ({ roles }) => {
			if (roles?.length) {
				return roles.map((item, index) =>
					h(NTag, { type: 'success', style: index > 0 ? 'margin-left: 8px;' : '' }, { default: () => item.name })
				)
			}
			return '暂无角色'
		}
	...
	{ title: '邮箱', key: 'profile.email', width: 150, ellipsis: { tooltip: true } },
	{
		title: '创建时间',
		key: 'createDate',
		width: 180,
...
async function handleEnable(row) {
	...
	try {
		await api.update(row.id, { enable: !row.enable })
